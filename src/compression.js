...
/** Provides a function that compresses buffers using the UO pre-defined
 * huffman tree.
 * 
 * @module Compression
 */
...
/** Compresses one buffer into another.
 * 
 * @param {Buffer} src The source buffer to compress.
 * @param {Buffer} dest The destination buffer to compress into.
 * @returns {Number} The length of valid data in dest.
 */
function compress(src, dest) {
    ...
    for(var iSrc = 0; iSrc < src.length; ++iSrc) {
        byte = src.readUInt8(iSrc);
        huffmanIndex = byte << 1;
        bitCount += huffmanTable[huffmanIndex];
        bitValue <<= huffmanTable[huffmanIndex];
        bitValue |= huffmanTable[huffmanIndex + 1];
        while(bitCount >= 8) {
            bitCount -= 8;
            dest.writeUInt8((bitValue >> bitCount) & 0xff, destIndex++);
        }
    }
    huffmanIndex = 0x200;
    bitCount += huffmanTable[huffmanIndex];
    ...
    if((bitCount & 7) !== 0)
    {
        bitValue = bitValue << (8 - (bitCount & 7));
        bitCount = bitCount + (8 - (bitCount & 7));
    }
    while(bitCount >= 8) {
        bitCount -= 8;
        dest.writeUInt8((bitValue >> bitCount) & 0xff, destIndex++);
